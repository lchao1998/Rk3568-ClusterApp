...
class TelltaleDataModel : public DataModel
{
    Q_OBJECT
public:
    explicit TelltaleDataModel(int updateInterval = 200, QObject *parent = nullptr);

protected:
    void updateData() override;

    DATA_PROPERTY_DEFINE(leftTurnlamp, bool) //左转弯灯
    DATA_PROPERTY_DEFINE(rightTurnlamp, bool) //右转弯灯
    DATA_PROPERTY_DEFINE(hazardIsSignaling, bool) //危险报警灯
...
signals:
    CHANGED_SIGNAL_DEFINE(leftTurnlamp)
    CHANGED_SIGNAL_DEFINE(rightTurnlamp)
    CHANGED_SIGNAL_DEFINE(hazardIsSignaling)
